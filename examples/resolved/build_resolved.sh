...
echo "Build ${PROGRAM}"
cargo build --release --bin resolved
# cargo build --bin resolved

echo "Install ${PROGRAM} (/usr/local)"
cp ./target/release/resolved /usr/local/sbin/resolved
# cp ./target/debug/resolved /usr/local/sbin/resolved
